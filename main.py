...
def viewproduct():
    ...
    conn1 = mysql.connector.connect(user='bala', password='Bala@333!', host='localhost', database='creditcard')
...
def RNewProduct():
    if request.method == 'POST':
        ...
        conn = mysql.connector.connect(user='root', password='Bala@333!', host='localhost', database='creditcard')
        cursor = conn.cursor()
        cursor.execute(
...
def NewCard():
    ...
    conn1 = mysql.connector.connect(user='root', password='Bala@333!', host='localhost', database='creditcard')
    cur1 = conn1.cursor()
...
def RNewcard():
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO cardtb(UserName,CardNo,CardType,ExpiryMonth,ExpiryYear,CvNo,pin,Status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (uname,cno,ctype,exm,exy,cnvo,pin,status))
        conn.commit()
        conn.close()
        # return 'file register successfully'
...
def userlogin():
    ...
    if request.method == 'POST':
        ...
        session['count'] = 0
        conn = mysql.connector.connect(user='bala', password='Bala@333!', host='localhost', database='creditcard')
        cursor = conn.cursor()
...
def Remove():
    ...
    conn = mysql.connector.connect(user='bala', password='Bala@333!', host='localhost', database='creditcard')
    cursor = conn.cursor()
    cursor.execute("Delete from protb  where ProductId='" + pid + "'")
    conn.commit()
    conn.close()
    conn1 = mysql.connector.connect(user='bala', password='Bala@333!', host='localhost', database='creditcard')
    # cursor = conn.cursor()
...
def Book():
    if request.method == 'POST':
        ...
        cursor = conn1.cursor()
        # cursor.execute(
        #     "INSERT INTO booktb VALUES ('','" + Bookingid + "','" + pid + "','" + new_string + "','" + UserName + "','" + Mobile + "','" + Email + "','" + str(
        #         Qty) + "','" + str(Amount) + "','" + str(CardType) + "','" + str(CardNo) + "','" + str(
        #         CvNo) + "','" + str(date) + "','0')")
        cursor.execute(
    "INSERT INTO booktb (Bookid, Productid, productname, UserName, Mobile, Email, Qty, Amount, CardType, CardNo, CvNo, date, status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    (Bookingid, pid, new_string, UserName, Mobile, Email, Qty, Amount, CardType, CardNo, CvNo, date, '0'))
        conn1.commit()
        conn1.close()
        # return 'file register successfully'
